...
class R2Former(nn.Module):
    def __init__(self, img_size=(480, 640), decoder_embed_dim=32, decoder_depth=4, decoder_num_heads=4,
                 decoder_mlp_ratio=4., decoder_norm_layer=nn.LayerNorm, embed_dim=32, num_classes=2, num_patches=16,
                 input_dim=256, num_corr=5):
        super().__init__()
    ...
    def forward(self, x_global, x_rerank=None, y_global=None, y_rerank=None):
        ...
        x_rerank_token = F.normalize(x_rerank[:, :, 3:], p=2, dim=2)  # (1, 500, 128)
        y_rerank_token = F.normalize(y_rerank[:, :, 3:], p=2, dim=2)
        x_coordinate = x_rerank[:, :, :3].detach().clamp(min=0, max=1)  # (1, 500, 3)
        y_coordinate = y_rerank[:, :, :3].detach().clamp(min=0, max=1)

        correlation = torch.matmul(x_rerank_token, y_rerank_token.permute((0, 2, 1)))  # (1, 500, 500)
        xy_matrix = torch.cat(
            ...
            dim=3)  # (1, 500, 500, 7)
        order_q = torch.argsort(correlation.unsqueeze(3), dim=2, descending=True).repeat(1, 1, 1, 7)  # (1, 500, 500, 7)
        order_k = torch.argsort(correlation.unsqueeze(3), dim=1, descending=True).repeat(1, 1, 1, 7)
        select_q = torch.gather(input=xy_matrix, index=order_q[:, :, :self.num_corr, :], dim=2)  # (1, 500, num_corr, 7)
        select_k = torch.gather(input=xy_matrix, index=order_k[:, :self.num_corr, :, :], dim=1)  # (1, num_corr, 500, 7)
        ...
        select_k_copy[:, :, :, :6] = torch.flip(
            ...
            dims=(3,)).reshape(select_k.shape[0], select_k.shape[1], select_k.shape[2], 6)  # (1, num_corr, 500, 7)
        select = torch.cat([select_q, select_k.permute((0, 2, 1, 3))], dim=1)  # (1,1000, num_corr, 7)
        select_copy = torch.cat([select_q, select_k.permute((0, 2, 1, 3))], dim=1)  # (1, 1000, num_corr, 7)
        N_select = select.shape[1]

        pair_matrix = self.pair_head(select.reshape(B * N_select * self.num_corr, 7)).reshape(B * N_select,
                                                                                              self.num_corr,
                                                                                              self.decoder_embed_dim)
        # (1000,num_corr,384)
        # Transformer Block-1
        pair_matrix += get_2d_sincos_pos_embed_from_grid(self.decoder_embed_dim,
                                                         select_copy.reshape(B * N_select, self.num_corr, 7)[:, :,
                                                         3:5])  # (B * N_select , num_corr, decoder_embed_dim) 对数据库图片做位置编码
        x = torch.cat([self.cls_token_2.repeat(B * N_select, 1, 1), pair_matrix], dim=1)  # (1000,6,384)
        for blk in self.blocks_2:
            x = blk(x)
        x = self.decoder_norm(x)

        x = self.pair_head_2(x[:, 0, :].reshape(B * N_select, self.decoder_embed_dim)).reshape(B, N_select,
                                                                                               self.decoder_embed_dim)
        # (1,1000,384)

        x = x.reshape(B, N_select, self.decoder_embed_dim) + get_2d_sincos_pos_embed_from_grid(self.decoder_embed_dim,
                                                                                               select_copy[:, :, 0,
                                                                                               0:2])  # 对查询图片做位置编码
        ...
        for blk in self.blocks:
            x = blk(x)
        x = self.decoder_norm(x)  # 1,1001,384

        # predictor projection
        if self.num_classes == 1:
            local_score = self.decoder_pred(x[:, 0]).reshape(-1)  # (1,1)->(1)
            ...
            local_score = self.decoder_pred(x[:, 0])  # (1,2)
            final_score = global_score.detach() * self.ratio.clamp(min=0.1, max=0.9) + self.sm(local_score).detach()[:,
                                                                                       1] * (
                                  1 - self.ratio.clamp(min=0.1, max=0.9))  # (1,2)->(1)
        return local_score, final_score  # (1,2)  ,(1)
...
if __name__ == "__main__":
    ...
    device="cuda"
    x_global, y_global, x_rerank, y_rerank = torch.rand(10, 256), torch.rand(10, 256), torch.rand(10, 500,
                                                                                                  131), torch.rand(10,
                                                                                                                   500,
                                                                                                                   131)
    x_rerank=x_rerank.to(device="cuda")
    y_global=y_global.to(device="cuda")
    x_global=x_global.to(device="cuda")
    y_rerank=y_rerank.to(device="cuda")
    CE = torch.nn.CrossEntropyLoss(ignore_index=-100).to(device)
    rerank_out_pos, _ = model(x_global,x_rerank ,y_global,y_rerank)
    rerank_out_neg, _ = model(x_global, x_rerank, y_global, y_rerank)
    rerank_out_neg=rerank_out_neg.unsqueeze(-1)
    rerank_out_pos=rerank_out_pos.unsqueeze(-1)
    target = torch.zeros(rerank_out_pos.shape[0] * 2, dtype=torch.long).to(device)  # +num_pairs
    target[:rerank_out_pos.shape[0]] = 1
    rerank_loss = CE(torch.cat([rerank_out_pos, rerank_out_neg], dim=0),
                     target)
    torch.cuda.synchronize()
    time_start = time.time()
    flops, params = profile(model, inputs=(x_global,x_rerank,y_global,y_rerank))
    time_end = time.time()
