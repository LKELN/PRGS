...
class AttentionGraphLayer(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, learn_adj=True, dist_method='l2', norm_layer=partial(nn.LayerNorm, eps=1e-6),
        ...
        self.alpha = nn.Parameter(torch.ones(1) * 0.7, requires_grad=True)
    ...
    def get_adj_matrix(self, all_features):
        ...
        if self.dist_method == 'dot':
            ...
            distmat = torch.cdist(all_features, all_features, p=2)
    ...
    def forward(self, x, gallery_global, adj=None):
        """
...
class GrapgRerank(nn.Module):
    def __init__(self, infeature_dim, outfeature_dim, train_batch_size, rerank_batch_size,
        ...
        # self.active = nn.GELU()
        self.message_pass = MLP([256, 256, 256])
        # self.blocks=nn.ModuleList([Block(dim=infeature_dim,num_heads=4,mlp_ratio=4,qkv_bias=True) for _ in range(1)])
        # self.dim_linear = Mlp(in_features=infeature_dim)
    ...
    def forward(self, query_global, candidate_global=None):
        ...
        for i in range(self.num_graph_layers):
            differ_input = self.selfnode_graph_layers[i](differ_input, candidate_global)
        # differ_input=self.classifier(differ_input.transpose(-1,-2))
        if self.num_classes == 1:
            local_score = self.classifier(differ_input.transpose(-1,-2))
            ...
            local_score = self.classifier(differ_input.transpose(-1, -2))  # (2,11,2)
...
if __name__ == "__main__":
    ...
    rerank_out,_=model(query_global,candidate_global)

    rerank_out_pos = rerank_out[:, 0]
    rerank_out_neg = rerank_out[:, 1]
    target = torch.zeros(rerank_out_pos.shape[0] * 2).to(device)  # +num_pairs
