...
class AttentionGraphLayer(nn.Module):
    ...
    def get_adj_matrix(self, all_features, query_feature=None):
        ...
        if len(all_features.shape)==4:
            B,N,L,C=all_features.shape
            if query_feature is None:
                q_repeat=all_features.repeat(1,1,L,1).view(B,N,L*L,C)
                k_repeat=all_features.repeat(1,1,1,L).view(B,N,L*L,C)
                q=torch.cat((q_repeat,k_repeat),dim=-1)
                e=self.leak(self.a(q)).view(B,N,L,L)
            else:
                q_repeat = query_feature.repeat(1, N, L, 1)
                k_repeat = all_features.repeat(1, 1, 1, L).view(B, N, L * L, C)
                q = torch.cat((q_repeat, k_repeat), dim=-1)
                e = self.leak(self.a(q)).view(B, N, L, L)
        elif len(all_features.shape)==3:
            B, L, C= all_features.shape

            q_repeat = all_features.repeat(1, L, 1).view(B,  L * L, C)
            k_repeat = all_features.repeat(1,  1, L).view(B,  L * L, C)
            q = torch.cat((q_repeat, k_repeat), dim=-1)
            e = self.leak(self.a(q)).view(B,  L, L)

        adj_matrix = self.sm(e)
...
class GrapgRerank(nn.Module):
    def __init__(self, infeature_dim, outfeature_dim, train_batch_size, rerank_batch_size,
        ...
        for i in range(num_graph_layers):
            ...
            outfeature_dim=32
            # Append selfnode graph layers
            self.selfnode_graph_layers.append(
                AttentionGraphLayer(in_dim=infeature_dim, out_dim=outfeature_dim, learn_adj=learn_adj,
                                    ...
                                    mode="self"))
            infeature_dim=outfeature_dim
            # Append crossnode graph layers
            self.crossnode_graph_layers.append(
                AttentionGraphLayer(in_dim=infeature_dim, out_dim=outfeature_dim, learn_adj=learn_adj,
                                    dist_method=dist_method,
                                    norm_layer=norm_layer,
                                    num_heads=graph_num_heads,
            ...
            # Append image graph layers
            self.image_graph_layers.append(
...
def getGcnRerank(args):
    ...
    # args.dist_method = "l2"
    # args.num_classes = 2
    # args.rerank_batch_size = 2
    # args.num_graph_layers = 3
    # args.attention_depth = 1
...
if __name__ == "__main__":
    ...
    model.to(device)
    query_global, candidate_global, x_rerank = torch.rand(1, 256), torch.rand(100, 256), torch.rand(1, 10, 1200, 131)
