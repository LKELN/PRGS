...
class GeoLocalizationNetRerank(nn.Module):
    ...
    def forward_cnn(self, x):
        ...
        local_features = feature_reshape

        HW = max(H, W)
        # HW = 512.

        x_xy = torch.arange(0, self.multi_out, device=local_features.device).unsqueeze(0).expand(last_map.shape[0], -1)
    ...
    def forward_deit(self, x):
        ...
        if self.single:
            ...
            HW = max(H, W)
            output = output[:, 2:, :]
            x_xy = torch.arange(0, self.multi_out, device=output.device).unsqueeze(0).expand(last_map.shape[0], -1)
            x_xy = torch.cat([(x_xy.unsqueeze(2) % np.ceil(W / 16).astype(int) * 16 + 8) / 1. / HW,
                              (x_xy.unsqueeze(2) // np.ceil(W / 16).astype(int) * 16 + 8) / 1. / HW],
                             dim=2)
            x_attention = (last_map / torch.max(last_map, dim=1, keepdim=True)[0]).reshape(x_xy.shape[0],
