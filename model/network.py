...
from model.GCNRerank_v1 import getGcnRerank
from model.R2Former import R2Former
# from model.global_rerank import getGcnRerank
...
class GeoLocalizationNetRerank(nn.Module):
    ...
    def forward_deit(self, x):
        ...
        B, _, H, W = x.shape
        x_ori = x.detach()
        x = self.backbone.patch_embed(x)  # (1,1200,384)

        cls_tokens = self.backbone.cls_token.expand(B, -1, -1)  # stole cls_tokens mimpl fro Phil Wang, thanks
        ...
        if self.single:
            ...
            # order=torch.arange(0,1200,device=output.device).unsqueeze(0).expand(last_map.shape[0],-1)
            # output=output[:,2:,:]
            ...
            HW = max(H, W)
            output = output[:, 2:, :]
            x_xy = torch.arange(0, 1200, device=output.device).unsqueeze(0).expand(last_map.shape[0], -1)
            x_xy = torch.cat([(x_xy.unsqueeze(2) % np.ceil(W / 16).astype(int) * 16 + 8) / 1. / HW,
                              (x_xy.unsqueeze(2) // np.ceil(W / 16).astype(int) * 16 + 8) / 1. / HW],
                             dim=2)
            x_attention = (last_map / torch.max(last_map, dim=1, keepdim=True)[0]).reshape(x_xy.shape[0],
                                                                                           x_xy.shape[1], 1)
            multi_out = self.multi_out
            if self.args.finetune:
                local_features = self.local_head(output.reshape(B * multi_out, -1)). \
