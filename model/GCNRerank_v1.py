...
def get_integral_feature(feat_in, Hin=30, Win=40):
    """
    ...
    """
    # N, K * C, H, W
    B, N, HW, C = feat_in.shape  # 30,40
    feat_in = feat_in.transpose(-1, -2)
    feat_in = feat_in.reshape(B, N, C, Hin, Win)
    feat_out = torch.cumsum(feat_in, dim=-1)  # 按行累加
    feat_out = torch.cumsum(feat_out, dim=-2)  # 按列累加
    feat_out = torch.nn.functional.pad(feat_out, (1, 0, 1, 0), "constant", 0)
    # feat_out = torch.nn.functional.pad(feat_out, (0, 0,1, 0), "constant", 0)
    # 在左边和上面填充一排0
...
def get_square_regions_from_integral(feat_integral, patch_size, patch_stride):
    ...
    """
    B, N, D, H, W = feat_integral.shape
    feat_integral = feat_integral.reshape(B * N, D, H, W)
    ...
    conv_weight[:, :, -1, 0] = -1
    # print(conv_weight)
    feat_regions = torch.nn.functional.conv2d(feat_integral, conv_weight, stride=patch_stride, groups=D,
                                              dilation=patch_size)
    # groups作用：对input中的每个样例数据，将通道分为groups等份，即每个样例数据被分成了groups个大小为(in_channel/groups, iH, iW)的子数据。
    # groups决定了卷积后通道是否合并
    # 对于这每个子数据来说，卷积核的大小为(in_channel/groups, kH, kW)。这一整个样例数据的计算结果为各个子数据的卷积结果拼接所得
    # 卷积核通道数为1
    # dilation = patch_size,膨胀系数，核元素之间的距离，只膨胀一次
    feat_regions = feat_regions / (patch_size ** patch_size)
    Hout = (H - patch_size - 1) // patch_stride + 1
    Wout = (W - patch_size - 1) // patch_stride + 1
    feat_regions = feat_regions.reshape(B, N, D, Hout, Wout).flatten(3).transpose(-1, -2)
    return feat_regions, Hout, Wout
...
class GrapgRerank(nn.Module):
    def __init__(self, infeature_dim, outfeature_dim, train_batch_size, rerank_batch_size,
                 ...
                 patch_size=[3, 2], strides=[2, 1], paddings=[2, 1]):
        ...
        self.h = 30
        self.w = 40
        for kernel_size, stride, padding in zip(patch_size, strides, paddings):
            self.h = (self.h + 2 * padding - kernel_size) // stride + 1
            self.w = (self.w + 2 * padding - kernel_size) // stride + 1
        self.num_local_dyn = 1200
        self.groups = [infeature_dim, 1]
        ...
        self.conv_list = nn.ModuleList([nn.Sequential(
            nn.Conv2d(in_channels=infeature_dim, out_channels=infeature_dim, padding=padding, kernel_size=kernel_size,
                      stride=stride, groups=group),
            LayerNorm2d(infeature_dim),
            nn.GELU(),
            # nn.Conv2d(in_channels=infeature_dim, out_channels=infeature_dim, kernel_size=1,
            #           stride=1),
            # LayerNorm2d(infeature_dim),
            # nn.GELU()
        )
            for kernel_size, stride, padding, group in zip(patch_size, strides, paddings, self.groups)])
        self.conv_merge = nn.Conv2d(in_channels=infeature_dim, out_channels=infeature_dim, padding=1, kernel_size=3,
                                    stride=2, groups=infeature_dim)
    ...
    def forward(self, query_global, candidate_global=None, x_rerank=None, adj=None):
        ...
        for i in range(self.num_graph_layers):
            # if i > 0:
            #     B, N, HW, C = x_rerank.shape  # 30,40
            #     x_rerank = x_rerank.transpose(-1, -2)
            #     x_rerank = x_rerank.reshape(B * N, C, Hout, Wout)
            #     x_rerank = self.conv_list[i-1](x_rerank)
            #     _, D, Hout, Wout = x_rerank.shape
                # x_rerank = x_rerank.reshape(B, N, D, Hout, Wout).flatten(3).transpose(-1, -2)
            #
        ...
        # x_rerank = x_rerank.transpose(-1, -2)
        # x_rerank = x_rerank.reshape(B * N, C, Hout, Wout)
        # x_rerank = self.conv_merge(x_rerank)
        # _, D, Hout, Wout = x_rerank.shape
        # x_rerank = x_rerank.reshape(B, N, D, Hout, Wout).flatten(3).transpose(-1, -2)
...
if __name__ == "__main__":
    ...
    # model=torch.load(r"S:\pycharm\pythonProject\R2Former\logs\best_model.pth")
