...
def test_rerank(args, eval_ds, model, test_method="hard_resize", pca=None, num_local=500, rerank_dim=131,
    ...
    with torch.no_grad():
        for query_index in tqdm(range(0, predictions.shape[0], rerank_bs), ncols=100):
            ...
            query_re_inputs =torch.tensor(query_re_inputs).cuda()
            candidate_re_inputs=torch.tensor(candidate_re_inputs).cuda()
            rerank_feature=torch.cat((query_re_inputs,candidate_re_inputs),dim=1).to(torch.float32)
