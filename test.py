...
def test(args, eval_ds, model, test_method="hard_resize", pca=None):
    ...
    with torch.no_grad():
        ...
        logging.debug("Extracting database features for evaluation/testing, extraction time for each{}".format(
            extraction_time / eval_ds.database_num))
        queries_infer_batch_size = 1 if test_method == "single_query" else args.infer_batch_size
        eval_ds.test_method = test_method
        queries_subset_ds = Subset(eval_ds,
                                   list(range(eval_ds.database_num, eval_ds.database_num + eval_ds.queries_num)))
        ...
        for inputs, indices in tqdm(queries_dataloader, ncols=100):
            ...
            if test_method == "nearest_crop" or test_method == 'maj_voting':  # store the features of all 5 crops
                ...
                all_features[indices.numpy(), :] = features
            extraction_time+=(time.time()-t_s)
            print('extraction time for each', extraction_time / eval_ds.queries_num, extraction_time,
                  eval_ds.queries_num)
            logging.debug("Extracting database features for evaluation/testing, extraction time for each{}".format(
                extraction_time / eval_ds.queries_num))
    ...
    positives_per_query = eval_ds.get_positives()
    # args.recall_values by default is [1, 5, 10, 20]
    if args.dataset_name == 'msls.json':
        predictions_str = []
        for query_index, pred in enumerate(predictions):
        ...
        if eval_ds.split == 'test':
            ...
            recalls_str, recalls = eval_api(predictions_str, ks=args.recall_values,
                                            root_default=eval_ds.dataset_folder)  # eval_ds.dataset_folder
            return recalls, recalls_str
...
def test_rerank(args, eval_ds, model, test_method="hard_resize", pca=None, num_local=500, rerank_dim=131,
                rerank_top=100, rerank_bs=4, save=None, reg_top=5, ransac=False, threshold=0, debug=False):
    """Compute features of the given dataset and compute the recalls."""
    ...
    with torch.no_grad():
        ...
        for inputs, indices in tqdm(database_dataloader, ncols=100):
            ...
            extraction_time += (time.time() - t_s)
        logging.debug("Extracting queries features for evaluation/testing, time for each: {}".format(
            extraction_time / eval_ds.database_num))
        print('\nextraction time each:{}s,extraction time {},database_num {}.'.format(
            extraction_time / eval_ds.database_num, extraction_time, eval_ds.database_num))
        queries_infer_batch_size = 1 if test_method == "single_query" else args.infer_batch_size
        eval_ds.test_method = test_method
        queries_subset_ds = Subset(eval_ds,
                                   list(range(eval_ds.database_num, eval_ds.database_num + eval_ds.queries_num)))
    ...
    with torch.no_grad():
        for query_index in tqdm(range(0, predictions.shape[0], rerank_bs), ncols=100):
            ...
            query_inputs = queries_features[
                           query_index:min(predictions.shape[0], query_index + rerank_bs)]  # num_query,256 # len=1
            query_re_inputs = queries_re_features[
                              query_index:min(predictions.shape[0], query_index + rerank_bs)]  # (2,500,128)
            candidate_index = predictions[query_index:min(predictions.shape[0], query_index + rerank_bs),
                              :rerank_top]  # (1,100)
            candidate_re_inputs = database_re_features[candidate_index.reshape(-1)]  # (200,500,131)
            candidate_inputs = database_features[candidate_index.reshape(-1)]  # (100,256)
            if args.rerank_model == "r2former":

                query_inputs_expand = np.tile(np.expand_dims(query_inputs, 1), [1, rerank_top, 1]).reshape(
                    [-1, queries_features.shape[-1]])  # (100,256)

                query_re_inputs_expand = np.tile(np.expand_dims(query_re_inputs, 1), [1, rerank_top, 1, 1]).reshape(
                    [-1, queries_re_features.shape[-2], queries_re_features.shape[-1]])  # (100,500,131)
                ...
                # (100,2) ,(100)
                rerank_score = sm(rerank_score_ori)[:, 1]  # (100,1)
                rerank_score = torch.reshape(rerank_score, [query_re_inputs.shape[0],
                                                            rerank_top]).detach()  # .cpu().numpy()  # .softmax(dim=1)
            elif args.rerank_model=="GCNRerank":
                query_re_inputs=np.expand_dims(query_re_inputs,axis=1)
                candidate_re_inputs=candidate_re_inputs.reshape(query_re_inputs.shape[0],rerank_top,num_local,rerank_dim)
                rerank_feature = np.concatenate((query_re_inputs,candidate_re_inputs),axis=1)
                # for i in range(query_re_inputs.shape[0]):
                #     rerank_feature.append(
                #         np.expand_dims(np.concatenate(
                #             [np.expand_dims(query_re_inputs[i],axis=0), candidate_re_inputs[i * rerank_top:(i + 1) * rerank_top]],
                #             axis=0),axis=0))
                # rerank_feature = np.concatenate(rerank_feature,axis=0)
                query_inputs_cuda = torch.tensor(query_inputs).cuda()
                rerank_feature = torch.tensor(rerank_feature.astype(np.float32)).cuda()
                candidate_inputs_cuda = torch.tensor(candidate_inputs).cuda()
                time_s = time.time()
                rerank_score, final_score = Reranker(query_inputs_cuda, candidate_inputs_cuda, rerank_feature)
                # (2,100,2)  ,(2,11,1)
                rerank_score = sm(rerank_score)[:, :, 1].detach()
            ...
            #     #     rerank_feature.append(
            #     #         np.expand_dims(np.concatenate(
            #     #             [np.expand_dims(query_re_inputs[i], axis=0),
            #     #              candidate_re_inputs[i * rerank_top:(i + 1) * rerank_top]],
            #     #             axis=0), axis=0))
            #     # rerank_feature = np.concatenate(rerank_feature, axis=0)
            #     query_inputs_cuda = torch.tensor(query_inputs).cuda()
            #     # rerank_feature = torch.tensor(rerank_feature.astype(np.float32)).cuda()
            #     candidate_inputs_cuda = torch.tensor(candidate_inputs).cuda()
            #     time_s = time.time()
            #     rerank_score, final_score = Reranker(query_inputs_cuda, candidate_inputs_cuda)
            #     if args.num_classes==1:
            #         rerank_score = torch.sigmoid(rerank_score).detach()
    ...
    logging.debug(
        f"The rerank model is {args.rerank_model},{new_rank.shape}, {rerank_score.shape}, time:{time.time() - time_s}, time each:, {rerank_time / predictions.shape[0]}")
    ...
    if args.dataset_name == 'msls.json':
        ...
        if eval_ds.split == 'test':
            ...
            recalls_str, recalls = eval_api(np.array(predictions_str), ks=args.recall_values,
                                            root_default=eval_ds.dataset_folder)  # eval_ds.dataset_folder
