...
class BaseDataset(data.Dataset):
    ...
    def __init__(self, args, datasets_folder="datasets", dataset_name="pitts30k", split="train"):
        ...
        if dataset_name == 'msls.json':
            ...
            self.pIdx = np.load(os.path.join(self.dataset_folder, 'npys', 'msls_' + split + '_pIdx.npy'),
                                allow_pickle=True)
            self.nonNegIdx = np.load(os.path.join(self.dataset_folder, 'npys', 'msls_' + split + 'nonNegIdx.npy'),
                                     allow_pickle=True)
...
class TripletsDataset(BaseDataset):
    ...
    def __init__(self, args, datasets_folder="datasets", dataset_name="pitts30k", split="train", negs_num_per_query=10):
        ...
        if self.mining == "full":  # "Full database mining" keeps a cache with last used negatives
            ...
            self.neg_cache = np.load("result/"+
                args.dataset_name + '_v2_' + args.backbone + '_hard_final_distance.npy')  # _hard_final_strict
