...
def run_train():
    ...
    for epoch_num in range(start_epoch_num, args.epochs_num):
        ...
        for loop_num in range(loops_num):
            ...
            writer.add_scalar('Train/current batch triplet loss', batch_loss, (epoch_num + 1) * (loop_num+1))
            writer.add_scalar('Train/average epoch triplet loss', epoch_losses.mean(), (epoch_num + 1) * (loop_num+1))

        logging.info(f"Finished epoch {epoch_num:02d} in {str(datetime.now() - epoch_start_time)[:-7]}, "
                     f"average epoch triplet loss = {epoch_losses.mean():.4f}")
        writer.add_scalar('Train/AvgLoss', epoch_losses.mean(), epoch_num + 1)
